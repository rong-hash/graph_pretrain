...
from construct_data import construct_pre_data
...
def load_graphons(split,pre_splits,down_generator,dataset):
    pre_graphons = []
    if split == "topo":
        cluster_num = 5
    elif split == "domain":
    ...
    for i in range(cluster_num):
        if args.func:
            if split == "trivial":
                load_path = "data/graphons/" + split + "/"+dataset+ pre_splits + "/func.npy"
            elif split == "domain":
                load_path = "data/graphons/" + split + "/"+dataset+ pre_splits[i] + "/func.npy"
            else:
                load_path = "data/graphons/" + split +  "/"+dataset+ pre_splits + + "/func"+str(
                    i) + ".npy"
            ...
            if split == "trivial":
                load_path = "data/graphons/" + split + "/"+dataset+ pre_splits + "/graphon.npy"
            elif split=="domain":
                load_path = "data/graphons/" + split +  "/"+dataset+ pre_splits[i]  + "/graphon.npy"
            else:
                load_path = "data/graphons/" + split +  "/"+dataset+ pre_splits + "/graphon"+ str(
                    i) + ".npy"
            graphon = np.load(load_path)
...
for k in range(len(splits)):
    split = splits[k]
    pre_graphons, cluster_num, down_generator = load_graphons(split,pre_split,down_generator,args.pre_data)
